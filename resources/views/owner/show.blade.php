...
@section('content')
    ...
    <div class="mb-3">
        {{-- PERBAIKAN: Mengubah route ke 'owner.kost.index' --}}
        <a href="{{ route('owner.kost.index') }}" class="btn btn-secondary btn-sm"><i class="bx bx-arrow-back"></i> Kembali</a>
    </div>
                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        ...
                        <div class="d-flex gap-2">
                            ...
                            <form action="{{ route('owner.kost.destroy', $kost->id) }}" method="POST"
                            ...
                            @foreach ($kost->foto as $key => $foto)
                                <div class="position-relative d-inline-block">
                                    <button type="button" data-bs-target="#kostCarousel"
                                        data-bs-slide-to="{{ $key }}" class="border-0 bg-transparent p-0">
                                        <img src="{{ asset('storage/' . $foto->foto) }}" class="rounded border shadow-sm"
                                            style="width:120px;height:90px;object-fit:cover;cursor:pointer;">
                                    </button>

                                    {{-- PERBAIKAN: Mengubah form action ke 'owner.kost-foto.destroy' --}}
                                    <form action="{{ route('owner.kost-foto.destroy', $foto->id) }}" method="POST"
                    ...
                    @else
                        <img src="{{ asset('template/paneladmin/assets/img/background/1.jpg') }}" class="img-fluid rounded"
        ...
        <div class="col-md-4">
            ...
            <div class="card border-0 shadow-sm">
                <div class="card-body">
                    ...
                    {{-- PERBAIKAN: Mengubah form action ke 'owner.kost-foto.store' --}}
                    <form action="{{ route('owner.kost-foto.store', $kost->id) }}" method="POST"
    ...
    <div class="modal fade" id="modalEditKost" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                {{-- PERBAIKAN: Mengubah form action ke 'owner.kost.update' --}}
                <form action="{{ route('owner.kost.update', $kost->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="modal-body">
                        <div class="mb-3">
                            ...
                            <input type="text" name="nama_kost" value="{{ $kost->nama_kost }}" class="form-control"
                                required>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" name="harga" value="{{ $kost->harga }}" class="form-control"
                                required>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="jenis_kost_id" class="form-select" required>
                                @foreach ($jenis as $j)
                                    <option value="{{ $j->id }}"
                                        {{ $kost->jenis_kost_id == $j->id ? 'selected' : '' }}>
                                        {{ $j->jenis_kost }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="daerah_kost_id" class="form-select" required>
                                @foreach ($daerah as $d)
                                    <option value="{{ $d->id }}"
                                        {{ $kost->daerah_kost_id == $d->id ? 'selected' : '' }}>
                                        {{ $d->name }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            @foreach ($fasilitas as $f)
                                <div class="form-check form-check-inline me-3">
                                    <input class="form-check-input" type="checkbox" name="fasilitas[]"
                                        value="{{ $f->id }}" id="f-{{ $f->id }}"
                                        {{ $kost->fasilitas->contains($f->id) ? 'checked' : '' }}>
                            ...
                            @foreach ($keamanan as $dk)
                                <div class="form-check form-check-inline me-3">
                                    <input class="form-check-input" type="checkbox" name="keamanan[]"
                                        value="{{ $dk->id }}" id="k-{{ $dk->id }}"
                                        {{ $kost->keamanan->contains($dk->id) ? 'checked' : '' }}>
                                    <label class="form-check-label"
                                        for="k-{{ $dk->id }}">{{ $dk->keterangan }}</label>
                                </div>
                            @endforeach
                        </div>
                            ...
                            @foreach ($kebersihan as $kb)
                                <div class="form-check form-check-inline me-3">
                                    <input class="form-check-input" type="checkbox" name="kebersihan[]"
                                        value="{{ $kb->id }}" id="kb-{{ $kb->id }}"
                                        {{ $kost->kebersihan->contains($kb->id) ? 'checked' : '' }}>
                                    <label class="form-check-label"
                                        for="kb-{{ $kb->id }}">{{ $kb->keterangan }}</label>
                                </div>
                            @endforeach
                        </div>
