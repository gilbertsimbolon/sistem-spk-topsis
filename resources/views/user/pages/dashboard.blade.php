                        ...
                        <a href="{{ route('dashboard.kost.show', $kos->id) }}" class="text-decoration-none text-dark d-block h-100">
                            <div class="card border-0 shadow-sm h-100 kost-card">
                                <div class="position-relative">
                                    <img src="{{ $kos->foto->isNotEmpty() ? asset('storage/' . $kos->foto->first()->foto) : asset('template/paneladmin/assets/img/background/1.jpg') }}" class="card-img-top kost-img" alt="{{ $kos->nama_kost }}">
                        ...
                        <div class="row g-3 align-items-end">
                            ...
                            <div class="col-md-3">
                                ...
                                <select class="form-select filter-input" name="daerah_kost_id" id="daerahSelect">
                                    ...
                                    @foreach ($daerah ?? [] as $d)
                                        <option value="{{ $d->id }}" {{ request('daerah_kost_id') == $d->id ? 'selected' : '' }}>
                                            {{ $d->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-3">
                                ...
                                <select class="form-select filter-input" name="tipe_kost">
                                    ...
                                    @foreach ($jenis ?? [] as $j)
                                        <option value="{{ $j->id }}" {{ request('tipe_kost') == $j->id ? 'selected' : '' }}>
                                            {{ $j->jenis_kost }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-3">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" id="harga_minimal" name="harga_minimal" class="form-control rupiah-field filter-input" value="{{ request('harga_minimal') ? 'Rp ' . number_format(request('harga_minimal'), 0, ',', '.') : '' }}" placeholder="Contoh: Rp 300.000">
                                </div>
                            </div>
                            ...
                            <div class="col-md-3">
                                ...
                                <div class="input-group">
                                    <span class="input-group-text bg-light"><i class="bi bi-wallet2 text-muted"></i></span>
                                    <input type="text" id="harga_maksimal" name="harga_maksimal" class="form-control rupiah-field filter-input" value="{{ request('harga_maksimal') ? 'Rp ' . number_format(request('harga_maksimal'), 0, ',', '.') : '' }}" placeholder="Contoh: Rp 1.000.000">
                                </div>
                            </div>
                            ...
                            <div class="collapse col-12 {{ request('fasilitas') || request('keamanan') || request('kebersihan') ? 'show' : '' }}" id="collapseKriteria">
                                            ...
                                            <div style="max-height: 150px; overflow-y: auto; padding-right: 5px;">
                                                @foreach ($fasilitas ?? [] as $f)
                                                    <div class="form-check mb-1">
                                                        {{-- PERBAIKAN: name diubah dari fasilitas[] menjadi cb_fasilitas --}}
                                                        <input class="form-check-input filter-input cb-fasilitas" type="checkbox" value="{{ $f->id }}" id="f-{{ $f->id }}" {{ is_array(request('fasilitas')) && in_array($f->id, request('fasilitas')) ? 'checked' : '' }}>
                                                        <label class="form-check-label small text-dark" for="f-{{ $f->id }}">{{ $f->keterangan }}</label>
                                                    </div>
                                                @endforeach
                                            </div>
                                        </div>
                                        ...
                                        <div class="criteria-box shadow-sm">
                                            ...
                                            <div style="max-height: 150px; overflow-y: auto; padding-right: 5px;">
                                                @foreach ($keamanan ?? [] as $k)
                                                    <div class="form-check mb-1">
                                                        {{-- PERBAIKAN: name diubah dari keamanan[] menjadi cb_keamanan --}}
                                                        <input class="form-check-input filter-input cb-keamanan" type="checkbox" value="{{ $k->id }}" id="k-{{ $k->id }}" {{ is_array(request('keamanan')) && in_array($k->id, request('keamanan')) ? 'checked' : '' }}>
                                                        <label class="form-check-label small text-dark" for="k-{{ $k->id }}">{{ $k->keterangan }}</label>
                                                    </div>
                                                @endforeach
                                            </div>
                                        </div>
                                        ...
                                        <div class="criteria-box shadow-sm">
                                            ...
                                            <div style="max-height: 150px; overflow-y: auto; padding-right: 5px;">
                                                @foreach ($kebersihan ?? [] as $b)
                                                    <div class="form-check mb-1">
                                                        {{-- PERBAIKAN: name diubah dari kebersihan[] menjadi cb_kebersihan --}}
                                                        <input class="form-check-input filter-input cb-kebersihan" type="checkbox" value="{{ $b->id }}" id="b-{{ $b->id }}" {{ is_array(request('kebersihan')) && in_array($b->id, request('kebersihan')) ? 'checked' : '' }}>
                                                        <label class="form-check-label small text-dark" for="b-{{ $b->id }}">{{ $b->keterangan }}</label>
                                                    </div>
                                                @endforeach
                                            </div>
                                        </div>
